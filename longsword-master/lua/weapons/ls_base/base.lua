...
function SWEP:SetupDataTables()
    -- ALWAYS define these FIRST so ExtraDataTables can't override them
    self:NetworkVar("Bool", 0, "Ironsights")
    self:NetworkVar("Bool", 1, "Reloading")
    self:NetworkVar("Bool", 2, "Bursting")
    self:NetworkVar("Bool", 3, "Lowered")   -- << GUARANTEED RESERVED
    ...
    -- Extra vars must be added AFTER reserved indices
    if self.ExtraDataTables then
        self:ExtraDataTables({
            ["Bool"] = 4,   -- start AFTER Lowered
            ["String"] = 1,
            ["Float"] = 5
        })
    end
